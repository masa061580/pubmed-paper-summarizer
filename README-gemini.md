# Pubmed論文自動要約アプリ (Gemini版)

このGAS（Google Apps Script）アプリケーションは、Pubmedの論文を定期的に検索し、新着論文のアブストラクトをGoogle Gemini 2.0 Flash APIで要約して、結果をスプレッドシートに記録し、メールで通知する機能を提供します。

## 機能

- PubmedのAPIを使用して特定の検索用語（ユーザーが指定）で週に1回検索
- 過去1週間にpublishされた論文のAbstractをGemini 2.0 Flash API（無料版）を使って簡潔に要約
...
- **無料で利用可能**: Gemini 2.0 Flash APIは無料枠が提供されています
- **高速処理**: Gemini 2.0 Flashは高速な応答が特徴です
- **簡単な取得**: APIキーはGoogleアカウントがあればすぐに取得できます
...
2. 適当な名前（例: "Pubmed論文自動要約 (Gemini版)"）を付けて保存します
...
2. 「API設定」タブでGemini APIキーを設定します
   - APIキーは [Google AI Studio](https://aistudio.google.com/app/apikey) から**無料**で取得できます
   - APIキーは「AIza」で始まる形式です
...
- Gemini APIには無料枠がありますが、使用量によっては制限される可能性があります。[Google AI Studioの利用規約](https://ai.google.dev/)を確認してください。
...
- Gemini APIキーが正しく設定されているか確認してください（「AIza」で始まる形式）
- Gemini APIの接続状態を確認してください
- APIキーの無料枠を超過していないか確認してください
...
このアプリケーションには[Claude版](https://github.com/masa061580/pubmed-paper-summarizer)も存在します。主な違いは以下の通りです：

| 項目 | Gemini版 | Claude版 |
|------|----------|----------|
| AIモデル | Gemini 2.0 Flash | Claude 3 Haiku |
| 無料枠 | あり | なし（有料） |
| APIキー取得 | Google AI Studio | Anthropic Console |
| 応答速度 | 高速 | 高速 |
| 要約品質 | 高品質 | 高品質 |
